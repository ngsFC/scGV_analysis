...
This analysis demonstrates the application of Joint Random Forest (JRF) for inferring gene regulatory networks from single-cell RNA-seq data of peripheral blood mononuclear cells (PBMCs). We utilize the scGraphVerse package to construct consensus networks, identify functional communities, and perform pathway enrichment analysis.

# Introduction

Gene regulatory networks (GRNs) provide crucial insights into cellular function and disease mechanisms. Single-cell RNA sequencing enables the reconstruction of these networks at unprecedented resolution. Here, we apply the Joint Random Forest methodology to infer robust gene regulatory networks from PBMC data.
...
# Set custom theme for publication-quality figures
...
seurat_subset_filt <- readRDS("PBMC.top600.RDS")

# Data summary
cat("Dataset Summary:\n")
cat("Number of cells:", ncol(seurat_subset_filt), "\n")
cat("Number of genes:", nrow(seurat_subset_filt), "\n")
cat("Number of donors:", length(unique(seurat_subset_filt$donor_id)), "\n")
```
...
```{r data-split}
# Split data by donor for robust network inference
...
```{r consensus-network}
# Create consensus network using voting approach
...
# Perform community detection and pathway analysis
...
# Filter communities with at least 10 genes for visualization
...
# Remove specific communities if needed (adjust based on your analysis)
#valid_communities <- valid_communities[!valid_communities %in% c(5, 6)]

# Get vertices in valid communities
valid_vertices <- names(membership)[membership %in% valid_communities]

# Create subgraph and convert to tidygraph
...
library(igraph)
# Create publication-quality network plot
set.seed(1234)
...
ggsave(file.path(plots_dir, "network_communities.png"), p1,
         width = 14, height = 10, dpi = 300, bg = "white", device = "png")
# Save network plot
ggsave(file.path(plots_dir, "network_communities.png"), p1, 
...
# Save degree distribution plots
...
```{r pathway-prep}
# Extract pathway results for major communities
selected_communities <- names(community_sizes[community_sizes >= min_community_size])
selected_communities <- selected_communities[!selected_communities %in% c("5", "6")]

# Combine pathway data with community information
...
# Filter and prepare pathway data
...
if (nrow(pathway_df) > 0) {
  # Select top pathways per community for visualization
  ...
  p4 <- ggplot(top_pathways, aes(x = Community, y = reorder(Description, log10_padj))) +
    ...
    theme_minimal() +  # Use theme_minimal() instead of theme_publication
    theme(
      text = element_text(size = 12),  # Remove family specification
  ...
  # Save top pathways data
  write.csv(top_pathways, file.path(tables_dir, "top_pathways.csv"), row.names = FALSE)
  
  # Pathway enrichment heatmap
  if (length(unique(top_pathways$Community)) > 1) {
    heatmap_data <- top_pathways %>%
      select(Community, Description, log10_padj) %>%
      pivot_wider(names_from = Community, values_from = log10_padj, values_fill = 0) %>%
      column_to_rownames("Description") %>%
      as.matrix()
    
    # Save heatmap data
    write.csv(heatmap_data, file.path(tables_dir, "pathway_heatmap_data.csv"), row.names = TRUE)
    
    # Save heatmap as PNG
    png(file.path(plots_dir, "pathway_enrichment_heatmap.png"), 
        width = 12, height = 8, units = "in", res = 300)
    p5 <- pheatmap(
      heatmap_data,
      color = viridis::plasma(50),
      cluster_rows = TRUE,
      cluster_cols = FALSE,
      fontsize = 10,
      fontsize_row = 8,
    ...
    # Save heatmap as PDF
    pdf(file.path(plots_dir, "pathway_enrichment_heatmap.pdf"), 
        width = 12, height = 8)
    p5 <- pheatmap(
      heatmap_data,
      color = viridis::plasma(50),
...
# Validate network edges against STRING database
cat("Validating network edges against STRING database...\n")

  # Then run your STRING code
...
# Process STRING adjacency matrix
...
```{r pubmed-mining}
cat("Performing literature mining for edge validation...\n")

# Query PubMed for edge support
...
if (length(pubmed_results) > 0 && !is.null(pubmed_results[[1]])) {
  ...
  # Filter edges by community and literature support
  filtered_edges_by_community <- lapply(names(community_genes), function(name) {
    genes <- community_genes[[name]]
    edges <- query_df %>%
      filter(gene1 %in% genes & gene2 %in% genes & pubmed_hits > 10) %>%
  ...
  literature_summary <- data.frame(
    ...
    High_Support = sapply(filtered_edges_by_community, function(x) sum(x$pubmed_hits > 50)),
    Medium_Support = sapply(filtered_edges_by_community, function(x) sum(x$pubmed_hits > 10 & x$pubmed_hits <= 50))
  ) %>%
  ...
  # Top supported edges per community
...
write.table(as.data.frame(summary_results), "../output/results_cases.txt", col.names = T, row.names = F, sep = "\t", quote = F)
```

This analysis demonstrates the power of Joint Random Forest for inferring robust gene regulatory networks from single-cell data. The identified communities show strong functional coherence and are supported by literature evidence, providing valuable insights into PBMC gene regulation patterns.
